...
//                    print( "section -> \(section ?? "nil"), algorithm -> \(algorithm ?? "nil"), rawType -> \(rawType ?? "nil"), encoding -> \(encoding ?? "nil")")
            ...
            it( "can parse sample attr change event" ) {
                ...
                let event = EthereumDIDRegistry.Events.DIDAttributeChanged.Arguments(identity: identityAddressSolidity, name: name, value: valueBytes!, validto: soonUInt256, previouschange: previousChangeUInt256)
            ...
            it( "can resolve real address" ) {
                ...
                let publicKeyEntry = PublicKeyEntry(id: "did:ethr:0xb9c5714089478a327f09197987f16f9e5d936e8a#owner", type: .Secp256k1VerificationKey2018, owner: "did:ethr:0xb9c5714089478a327f09197987f16f9e5d936e8a", ethereumAddress: "0xb9c5714089478a327f09197987f16f9e5d936e8a")
                let publicKey = [publicKeyEntry]
                let authenticationEntry = AuthenticationEntry(type: .Secp256k1SignatureAuthentication2018, publicKey: "did:ethr:0xb9c5714089478a327f09197987f16f9e5d936e8a#owner")
                let authentication = [authenticationEntry]
                
                let referenceDDO = DDO(id: id, publicKey: publicKey, authentication: authentication, service: [ServiceEntry](), context: context)
